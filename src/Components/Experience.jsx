    ...
    <div className="experience flex flex-col ">
      ...
      <div className="companies flex flex-col md:flex-row justity-space-between gap-6 px-10 py-4">
        {workExp.map((exp, index) => (
          <div
            key={index}
            className="content-box flex-1 px-4 py-2 w-full min-h-auto bg-cyan-950 rounded-lg overflow-hidden relative hover:mx-1 hover:border-2 border-white hover:brightness-110"
            style={{ transition: "transform 0.2s ease-in-out" }}
          >
            <div className="row-1 flex flex-col md:flex-row">
              <div className="details flex-1 flex flex-col px-4 py-4">
                <h2 className="text-xl md:text-4xl md:border-4 rounded-md md:w-fit md:px-4 md:py-2 md:bg-cyan-700 text-slate-100 my-2 ml-4 font-extrabold relative">
                  {exp.Company}
                  <img
                    src={exp.image}
                    alt="logo"
                ...
                <h4 className="text-2xl text-left md:text-4xl ml-4 text-slate-300 font-bold hover:text-stone-100">
                  {exp.role}
                </h4>
              </div>
              <img
